...
st.caption("Query Companies House registry and scrape primary web contacts to assemble lead dossiers.")
...
if fetch_btn and company_query:
    with st.spinner("Querying registry & parsing website..."):
        ...
        lead = enricher.build_lead_profile(company_query, website=website_input)
...
if "current_lead" in st.session_state:
    ...
    with col2:
        ...
        badge_cols[2].metric("SIC Code", ", ".join(lead.sic_codes) if lead.sic_codes else "Unknown")
        
        st.markdown(f"**Official Name:** {lead.company_name}")
        st.markdown(f"**Registered Address:** {lead.registered_address or 'Not found'}")
        ...
        with c_col1:
            ...
            if lead.officers:
                for off in lead.officers:
                    st.markdown(f"- **{off.name}** ({off.role})  \n  *Appointed: {off.appointed_on or 'N/A'}*")
            else:
                st.write("No active officers found via API.")
                
        with c_col2:
            st.markdown("**Scraped Contact Points:**")
            st.markdown("**Emails:** " + (", ".join(lead.emails_found) or "None detected"))
            st.markdown("**Phone Numbers:** " + (", ".join(lead.phones_found) or "None detected"))
            
        st.success("Data ingestion complete. Ready to pass this payload into the LLM Hook Engine.")
